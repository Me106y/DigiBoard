...
class MobileSegV2(nn.Layer):
    ...
    Args:
        ...
        use_enhanced_cm (bool, optional): Whether to use enhanced context module. Default: True.
    ...
    def __init__(self,
        ...
        self.ppseg_head = MobileSegHead(backbone_out_chs, arm_out_chs,
                                        ...
                                        resize_mode, use_last_fuse, use_enhanced_cm)
...
class MobileSegHead(nn.Layer):
    ...
    Args:
        ...
        use_enhanced_cm (bool): Whether to use enhanced context module. Default: True.
    """

    def __init__(self, backbone_out_chs, arm_out_chs, cm_bin_sizes, cm_out_ch,
                 arm_type, resize_mode, use_last_fuse, use_enhanced_cm=True):
        super().__init__()

        # 使用增强版上下文模块
        if use_enhanced_cm:
            self.cm = EnhancedMobileContextModule(
...
class EnhancedMobileContextModule(nn.Layer):
    """
    Enhanced Context Module for Mobile Model with improved multi-scale context aggregation.
    ...
    Args:
        ...
        use_aspp (bool): Whether to use lightweight ASPP branch. Default: True.
        use_global_context (bool): Whether to use global context attention. Default: True.
        align_corners (bool): An argument of F.interpolate. Default: False.
    """
    ...
    def __init__(self,
        ...
        if use_aspp:
            self.aspp_convs = nn.LayerList([
                layers.SeparableConvBNReLU(
                    in_channels, inter_channels, kernel_size=3,
                    dilation=rate, bias_attr=False
                ) for rate in [6, 12, 18]
            ])
        ...
        if use_global_context:
            self.global_context = nn.Sequential(
                ...
                layers.ConvBNReLU(in_channels, inter_channels // 4, kernel_size=1),
                nn.Conv2D(inter_channels // 4, in_channels, kernel_size=1),
                nn.Sigmoid()
            )

        # 特征融合
        total_channels = inter_channels * len(bin_sizes)
        if use_aspp:
            total_channels += inter_channels * 3  # 3个ASPP分支

        self.fusion_conv = layers.SeparableConvBNReLU(
    ...
    def forward(self, input):
        ...
        if self.use_global_context:
            global_context = self.global_context(input)
            input = input * global_context
        ...
        if len(feature_list) > 1:
            fused_features = paddle.concat(feature_list, axis=1)
