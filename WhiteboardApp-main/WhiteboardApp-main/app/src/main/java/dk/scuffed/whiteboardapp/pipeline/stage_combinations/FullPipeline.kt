...
internal fun fullPipeline(
...
): Pair<GLOutputStage, GLOutputStage> {


    val fullSegmentation = fullSegmentation(context, inputStage, pipeline)


    // 之前使用 perspectiveCorrected 作为输入
    val imageEnhancement = fullImageEnhancement(
        context,
        fullSegmentation,
        pipeline
    )
    ...
    return Pair(fullSegmentation, imageEnhancement)

//    return Pair(fullSegmentation, fullSegmentation)
}